...
exports.deleteaVehicle = async (req, res) => {
  const { id } = req.params;
  try {
    if (!mongoose.Types.ObjectId.isValid(id))
      return res.status(404).json({ msg: `No Vehicle with id:${id}` });

    await Vehicle.findByIdAndRemove(id);
    return res.status(204).json({ msg: "Vehicle Deleted Successfully" });
  } catch (err) {
    return res.status(500).json({ msg: "Something went wrong on Server.." });
  }
};
...
exports.editVehicle = async (req, res) => {
  ...
  try {
    ...
    const updatedVehicle = await Book.findByIdAndUpdate(
      id,
