...
exports.signUp = async (req, res) => {
  ...
  try {
    ...
    updatedUser.save();
    const sender = "reply.getvehicle@gmail.com";
    const subject = "Getvehicler Verify Email";
    const body =
    ...
    return res.status(200).json({profile: { vehicleNumber : vehicleNumber, email: newUser.email, id: newUser._id },token});
  } catch (err) {
...
exports.signIn = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      profile: { email: oldUser.email, id: oldUser._id },
      token,
    });
  } catch (err) {
    return res.status(500).json({ msg: "Something went wrong" });
